...
<c:choose>
	<c:when test="${role ne 'Volunteer'}">
		<html>
	       <head>
	          <script type="text/javascript">
	          alert("Only Volunteers has access to this dashboard");
	          location.href="/";
	          </script>
	       </head>
	   </html>
						...
						<tbody>
							<c:forEach items="${list }" var="fm">
								<tr>
									...
									<c:choose>
										<c:when test="${fm.status == 'Waiting for Pickup'}">
											<td><a href="/pickUp?fmid=${fm.fm_id }"
												class="btn btn-success">Pickup</td>
											</td>
										</c:when>
										<c:when test="${fm.status == 'Deliver In Progress'}">
											<td><a href="/delivered?fmid=${fm.fm_id }"
												class="btn btn-success">Delivered</td>
											</td>
										</c:when>
